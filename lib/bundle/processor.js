 ...
 *   Read → Minify HTML → Wrap (AMD) → Minify JS (Terser) → Write → Compress
 *
 ...
 * strategy is automatically downgraded to 'safe' to prevent fatal runtime errors
 * such as `$ is undefined`, missing closures, or broken payment gateway callbacks.
 *
 * @type {RegExp[]}
 */
 ...
 *   4. **Minify JS**: Run Terser with strategy-aware configuration.
 *   5. **Write**: Output the concatenated/minified bundle to the build directory.
 *   6. **Compress**: Generate `.gz`, `.br`, and `.zst` static compressed variants.
 *
 * @async
