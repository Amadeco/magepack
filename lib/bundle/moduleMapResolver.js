...
/**
 * Normalize a module path for map lookup.
...
/**
 * Try to resolve a mapped path from Magento's baseUrlInterceptor map.
 * Magento may store keys with or without ".js" depending on build mode.
 *
...
const resolveMappedValue = (map, modulePath) => {
    ...
    // Some setups include extension in the key
    if (!key.endsWith('.js') && map[`${key}.js`]) return map[`${key}.js`];

    // Some callers may already pass ".js" but map may not
    if (key.endsWith('.js')) {
 ...
 * If a module is mapped, the map value is a full replacement path containing the version hash.
 *
 * @param {string} themePath
 * @param {string} modulePath
 * @param {Record<string, string>} map
...
const mappedModulePath = (themePath, modulePath, map) => {
    ...
    // Map values are typically like "version/js/..." and already include modulePath.
