 ...
 * FIX: 
 * 1. Adds 'bundle-' prefix.
 * 2. Leaves 'deps' EMPTY. Pure Lazy Loading strategy.
 *
...
const buildRequireConfigContent = (config, isMinifyOn) => {
    const bundles = {};
    const deps = []; // EMPTY: We let the page/RequireJS decide when to load via on-demand require calls.
    ...
    config.forEach((bundle) => {
        ...
        // 1. Construct the path: 'magepack/bundle-name.min'
        ...
        // 3. Map the bundle path to its contained modules
