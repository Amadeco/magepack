...
C'est noté. Si le mode Safe fonctionne mais pas le mode Aggressive, cela confirme que la minification détruit l'ordre d'exécution ou supprime du code perçu à tort comme inutile (Side Effects) par Terser.

Le coupable est souvent l'option unused: true (qui peut supprimer des arguments de fonction dans les modules RequireJS, décalant ainsi les dépendances) ou side_effects: true (qui peut supprimer l'initialisation de plugins jQuery).

Voici la version corrective de lib/bundle.js.

Changements appliqués pour stabiliser le mode "Aggressive" :

    unused: false : CRITIQUE. Empêche Terser de supprimer les arguments de fonction non utilisés. Dans RequireJS, define(['a', 'b'], function(a, b) {}), si b est supprimé, RequireJS injecte quand même 2 arguments, ce qui peut créer des décalages.

    side_effects: false : Empêche Terser de supprimer des appels de fonction qu'il juge "purs" mais qui sont en fait nécessaires pour l'enregistrement de plugins (comme $.mage.translate).

    collapse_vars: false : Évite de fusionner des variables de manière trop complexe pour le vieux moteur JS de Magento.

    ecma: 2019 : Un poil plus conservateur que 2020 pour assurer la compatibilité avec les contextes this délicats de KnockoutJS.

Cela devrait vous donner les performances du mode Agressif (suppression des logs, syntaxe moderne) avec la fiabilité du mode Safe.
lib/bundle.js (Final Fix)
JavaScript
