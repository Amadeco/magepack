...
import createPathResolver from './bundle/moduleMapResolver.js'; // Gère le requirejs-map.js
 ...
 * @param {string} relativePath - Module path (may or may not have .js extension).
 ...
 */
const resolveFile = async (rootDir, relativePath, isMinified) => {
    // 1. Normalize: Remove .js extension to work with a raw "Module ID" concept
    // This fixes issues where we don't know if the input path has an extension or not.
    ...
    // 3. Construct Path
    const targetPath = path.join(rootDir, baseName + targetExtension);
    const fallbackPath = path.join(rootDir, baseName + '.js');
    ...
    try {
        ...
        if (isMinified && targetPath !== fallbackPath) {
            try {
                ...
                consola.debug(`Fallback used for ${baseName}: Found .js instead of .min.js`);
                return fallbackPath;
            } catch (e2) {
...
const processBundle = async (bundle, localePath, options) => {
    ...
    for (const moduleName of moduleNames) {
        ...
        try {
            // 1. Resolve logical path (handle Magento Versioning via requirejs-map)
            const mappedPath = resolveMap(rawModulePath);

            // 2. Resolve physical path (handle .min.js vs .js extensions strictly)
...
/**
 * Generates the RequireJS configuration wiring.
 * @param {Array} bundles 
 * @param {string} localePath 
 */
